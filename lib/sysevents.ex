defmodule Sysevents do
  ...
  defp get_chain(event_id, accumulator) do
    case get_link_from_db(event_id) do
      nil -> accumulator
      link-> get_parent_chain(link.parent_id, [link | accumulator]) ++ (get_child_chain(link.id, accumulator) |> Enum.reverse)  # NAUGHTY
    end
  end
  ...
  defp get_child_chain(event_id, accumulator) do
    case get_child_links_from_db(event_id) do
	[] -> accumulator
	kids -> 
          Enum.flat_map(kids, 
                        fn link -> 
                          get_child_chain(link.id, [link | accumulator])
                        end)
    end
  end

  defp get_child_links_from_db(event_id) do
    Event
    |> Ecto.Query.where(parent_id: ^event_id)
    |> Sysevents.Repo.all
